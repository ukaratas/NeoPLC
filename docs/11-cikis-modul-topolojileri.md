...
### Karar: ayrı modül aileleri, AC minimum 2U · 🟡 D-51

Bu bir tercih değil, fiziksel zorunluluk:

| Gerekçe | Detay |
|---------|-------|
| **Creepage / clearance** | 230V AC ile SELV (12–48V) arasında takviyeli izolasyon için ≥5.5 mm creepage. 22.5 mm'lik bir 1U modülde lojik + izolasyon + çok kanal yan yana sığmıyor |
...
**AC modülleri 2U.** Karavanda AC zaten yalnızca şebeke bağlıyken veya inverter
çalışırken var — kanal sayısı ihtiyacı DC'ye göre çok daha az.

---

## 5. Önerilen modül ailesi

### 5.1 DC çıkış

| Modül | Form | Kanal | Akım | Teknoloji | Hedef yük |
|-------|------|-------|------|-----------|-----------|
| **DO-DC-8A-L** | 1U | 8 | 8 A | **Latching röle** | Buzdolabı, ısıtıcı, sabit aydınlatma — **saatlerce açık kalanlar** |
| **DO-DC-8A-M** | 1U | 8 | 8 A | **MOSFET** (akıllı high-side) | Dimmer'lı aydınlatma, fan hızı, pompa soft-start — **PWM gerekenler** |
| **DO-DC-32A-H** | 1U | 2 | 32 A | **Hibrit** | Inverter beslemesi, su ısıtıcı, klima |
...
| Modül | Form | Kanal | Akım | Teknoloji | Not |
|-------|------|-------|------|-----------|-----|
| **DO-AC-16A-L** | 2U | 4 | 16 A | Latching röle | 16 A = standart şebeke girişi sınıfı |
| **DO-AC-32A-L** | 2U | 2 | 32 A | Latching röle | Yüksek güç / ana AC hat |

Senin önerinle aynı — AC'de latching tartışmasız doğru: sıfır tutma gücü **ve**
AC'de ark kesme zaten kolay (sıfır geçişi var).

### 5.3 Klemens yerleşimi kontrolü (2U = 45 mm)

```
16 A şebeke klemens adımı  7.5 mm  →  45 / 7.5 = 6 kutup
                                       4 kanal (L) + N + PE   ✓

32 A şebeke klemens adımı  10 mm   →  45 / 10 = 4 kutup
                                       2 kanal (L) + N + PE   ✓
```

Önerdiğin kanal sayıları boyutsal olarak tutuyor.
...
> ⚠️ **Düzeltme.** Bu bölümün ilk hali, modülün serbest havada tek başına
...
| Modül | Kayıp | ΔT (paket) | İç sıcaklık @60°C |
|-------|-------|------------|-------------------|
...
| Güç modüllerinin fan akışına yakın slotlara yerleştirilmesi | Host firmware'i slot tipini bildiği için kullanıcıya öneri verebilir |
| Modül içi sıcaklık sensörü | Aşırı ısınmada kanalı kendisi kapatır — descriptor yetenek bayrağı |
...
dizi. Modül MCU'sunda GPIO yakmıyor, maliyeti düşük.
...
Modül firmware'inin sorumluluğu. Örneğin 2 ms arayla → tüm kanallar 16 ms'de
tamamlanır, tepe akım 30 mA'de kalır.

Modül üzerinde bobin darbelerini besleyen yerel bir tampon kapasitör de
eklenmeli — darbe akımını backplane yerine kapasitörden çekmek rail'i korur.

### 7.3 Durum geri okuma

Latching röle **durumunu kaybetmez, ama yazılım kaybedebilir** — modül reset
olduğunda röle hâlâ eski konumunda.

**Zorunlu: kontak durumu geri okunabilmeli.** Yardımcı kontak veya çıkış
geriliminin ölçülmesi ile. Aksi halde modül açılışta rölenin durumunu bilemez ve
...
**DO-DC-64A-H (ana batarya kesici) yalnızca bir çıkış modülü değil, tüm
sistemin güvenlik yedeğidir.** Herhangi bir MOSFET kanalı kısa devre arızası
verirse, ana kesici tüm DC dağıtımı kesebilir.

Bu, 64 A modülü "opsiyonel yüksek akım kanalı" olmaktan çıkarıp **güvenlik
...
| ID | Soru |
|----|------|
| D-52 | 8 A sınıfında latching ve MOSFET **iki ayrı modül mü**, yoksa tek modülde karışık mı? Ayrı olması üretimi ve stoğu basitleştirir; karışık olması kullanıcıya esneklik verir. |
...
| D-54 | 32 A / 64 A modüllerde ısı yolu: alüminyum ön panel, kutu gövdesi, yoksa sadece PCB bakırı mı? |
| D-55 | AC modüllerinde sıfır geçişinde anahtarlama (zero-cross) uygulanacak mı? Kontak ömrünü uzatır, ek zamanlama devresi ister. |
