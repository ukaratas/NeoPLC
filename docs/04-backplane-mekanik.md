...
> Bu dokümandaki kararlar **geriye dönük değiştirilemez**. Üretilmiş her modül
> buraya bağlıdır.

## 1. Mekanik standart

### Karar: 1U = 22.5 mm · 2U = 45 mm · 🟡 D-15

Bu değer keyfi seçilmedi. **22.5 mm, endüstriyel DIN ray modül genişliğinin
...
8U toplam genişlik   8 × 22.5 = 180 mm   (+ master elektroniği bölümü)
```

### 1.2 Modül yönelimi

Modüller yan yana dizilen **dikey kartlardır** — PCB düzlemi komşusuna paralel.
...
| Boyut | Değer |
|-------|-------|
| Modül PCB yüksekliği | ~100 mm (standart DIN kutu iç yüksekliği) |
| Modül PCB derinliği | ~90 mm |
...
| Seçenek | Modül tarafı maliyet | Servis ömrü | Stagger | Değerlendirme |
|---------|---------------------|-------------|---------|---------------|
...
1. **Modül tarafı bedava** — konnektör diye bir parça yok, sadece PCB üzerinde
   gold finger. K1 (modül ucuz olmalı) doğrudan karşılanıyor.
...
| 5–6 | **VBUS_RAW** | → modül | 2 | 12–48V ham, slot başına 500 mA |
| 7–8 | **+5V_SYS** | → modül | 2 | Slot başına 200 mA (1U) / 400 mA (2U) |
...
| 17 | **MOD_RST#** | master → modül | 3 | Slot başına. Modülde 10k pull-**down** |
| 18 | **BOOT#** | master → tüm slotlar | 3 | Ortak. Reset bırakılırken örneklenir |
| 19 | **FAULT#** | modül → master | 3 | Ortak, open-drain wired-OR |
| 20 | **SYNC** | master → tüm slotlar | 3 | Eşzamanlı I/O latch strobe |
| 21–24 | *RSVD* | — | 3 | Gelecek genişleme |
| 25 | **PRESENT#** | modül → master | **4 (en kısa)** | GND'ye bağlı. **Son oturan pin** |
| 26 | **GND** | — | 1 | Ek dönüş |
...
**PRESENT#'in en son oturması tasarımın kilit taşı.** Master, PRESENT# düşene
kadar modülü yok sayar. Yarım oturmuş bir modül — güç ve sinyal kısmen bağlı —
sistemi hiç etkilemez, çünkü master onu görmez ve reset'ini bırakmaz.
...
| 21–24 | Bilinmeyen gelecek ihtiyaçları. 4 pinin maliyeti sıfıra yakın; sonradan eklemenin maliyeti tüm modül ailesi. |
...
## 4. 2U modül stratejisi

| Seçenek | Değerlendirme |
|---------|---------------|
| **(a) Sadece sol slotun konnektörüne oturur** | Tek konnektör hizalaması — mekanik olarak belirgin şekilde basit. Discovery'de "2U" bildirir, master N+1'i kapalı işaretler. |
| (b) Her iki konnektöre de oturur | 2× pin ve güç, ama iki konnektörün toleranslı hizalanması gerekir |

### Karar: (a) · 🟡 D-16

Güç yeterliliği kontrol edildi: 2U modülün 400 mA'i 2 adet +5V_SYS kontağından
geçiyor, 2.54 mm kart kenarı kontak kapasitesinin çok altında.

(b) sadece gelecekte yüksek güçlü bir 2U modül gerekirse açılacak — pinout'ta
bunu engelleyen bir şey yok.

### 4.1 Adresleme tutarlılığı

```
Slot 3–4'e takılı bir 2U modül:
  · ADDR pinlerinden slot 3'ü okur
  · Descriptor'da form_factor = 2U bildirir
  · Master slot 4'ü "2U tarafından kapatıldı" olarak işaretler
...
| Modül tarafı soft-start | ~$0.15 | |
| Hot-plug toleranslı RS-485 transceiver | **$0** | Aynı fiyat sınıfında mevcut |
| Protokol desteği | **$0** | Master-slave zaten timeout tabanlı; kaybolan modül = timeout, yeni modül = PCA9555 INT# |
| **Toplam ek maliyet** | **~$0.35 / modül** | |
...
|---|---|---|
| Maliyet | ~$0.35/modül | **Tüm modül ailesi + backplane yeniden tasarım** |
